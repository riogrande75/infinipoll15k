...
$moxa_port = 20108;
$moxa_timeout = 10;
$warte_bis_naechster_durchlauf = 2; //time between 2 query's in secs.
$antw="";
$tmp_dir = "/tmp/inv1/";             //temp folder for live values, -> at the end there must be a / !!!
if (!file_exists($tmp_dir)) {
        mkdir("/tmp/inv1/", 0777);
        }
$tmp_dir_backup = "/home/user/inv1/"; // folder for daily backup, pls. adapt to fit your filesystem
...
//logging/febugging settings:
$debug = 1;         //Debugoutput and Debuglogfile
$debug2 = 0;        //advanced debugging CLI only
$storage_stat=1;
$log2console = 0;
$fp_log = 0;
$script_name = "infinipoll50k_A.php";
$logfilename = "/etc/infinipoll15k/log/infinipoll_15k_A_";     //debugging logfile, folder must be created manually
...
syslog(LOG_ALERT,"**INFINIPOLL 15K A Neustart**");

if($debug) $fp_log = @fopen($logfilename.date("Y-m-d").".log", "a");
if($debug) logging("**INFINIPOLL 15K A Neustart**");

//check if there is BMS info in shared memory
$sh_bms = shmop_open(0x4801, "a", 0777, 57); //adapt shmop ID!
if (!$sh_bms) {
...
if($modelcode="000") $model="MPI Hybrid 15KW/3P";
$modelva = $antw[1];
$modelpf = $antw[2];
$modelbattpcs = $antw[7];
...
function sendsignal($message)
        ...
        shell_exec("/etc/signal/bin/signal-cli -u +4123123123 send -a /etc/infinipoll15k_A/alarm.png -m $message +41231231234");
        }
function parse_antw()
        {
        global $fp, $debug;
